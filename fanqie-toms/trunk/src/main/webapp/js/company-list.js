 ...
 */
$(".deletedCompany").bind("click",function(){
...
$('.permission-btn').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status) {
                $('.permission-user-id').val(data.company.id);
...
$('.btn-new-permission').on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status) {
                layer.alert('提示信息：新增公司成功', {icon: 6}, function () {
                    window.location.reload();
                });
            } else {
                layer.alert('提示信息：新增公司失败', {icon: 5}, function () {
                    window.location.reload();
                });
            }
        }, error: function () {
