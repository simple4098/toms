...
$('#myButton').on('click', function() {
	var tagId = $('#kz-tags-r').val(), accountId = $('#roomTypeName').val(),page = $('#pageNo').val();
	var postData = {'tagId': tagId, 'accountId': accountId, 'page':page};
	$.ajax({
		type: 'post',
		data: postData,
		url: "/oms/inn/obtRoomType" + "?v" + new Date().getTime(),
		dataType: 'html',
		success: function (data) {
			$("#roomTypeContainerId").html(data);
		}
...
function getDetail(accountId){
	$('#kz_item-r').val(accountId);
	var startDate = $('#from_datepicker').val(), endDate = $('#to_datepicker').val(), tagId = $('#kz-tags-r').val();
	var maiAccount = $(".maiAccount:checked").val();
	var postData = {'startDate': startDate, 'endDate': endDate, 'tagId': tagId, 'accountId': accountId,'maiAccount':maiAccount};
	getRoomType( postData );
	$("#manualOrder").show();
